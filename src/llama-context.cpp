...
void llama_context::sched_reserve() {
    ...
    for (size_t i = 0; i < backend_ptrs.size(); ++i) {
        ...
        if (backend_buf_exp_size[i] > 1) {
            ...
            fprintf(stderr, "COMPBUF: %-14s %8.2f MiB\n", ggml_backend_buft_name(buft),
                    backend_buf_exp_size[i] / 1024.0 / 1024.0);
            fflush(stderr);
        }
    }
...
llama_context * llama_init_from_model(
    ...
    if ((model->hparams.is_mla() || model->arch == LLM_ARCH_DEEPSEEK4) && params.type_k != params.type_v) {
        // fork: MLA caches are K-only (V is a view of K), so a mismatched V type is
        // meaningless rather than unsupported. Align it to K instead of rejecting so
        // existing -ctk-only command lines (e.g. -ctk q8_0) keep working.
        LLAMA_LOG_WARN("%s: model uses a K-only (MLA) cache - aligning V cache type to K (%s)\n",
