...
struct ggml_backend_meta_split_state llama_meta_device_get_split_state(const struct ggml_tensor * tensor, void * userdata) {
    ...
    if (split_state.axis >= 0 && split_state.axis < GGML_MAX_DIMS) {
        ...
        // hybrid TP x PP: -ts controls the layer distribution across the TP groups;
        // within a group the tensor split is always even
        static const bool tp_group_even = getenv("LLAMA_TP_GROUP_SIZE") != nullptr;
        if (tp_group_even) {
...
llama_memory_i * llama_model::create_memory(const llama_memory_params & params, const llama_cparams & cparams) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_GLM_DSA:
            // Fork policy for GLM-5.2 sparse attention:
            //   LLAMA_GLM_SPARSE set non-zero -> this fork's trained DSA path
            //     (deepseek32 graph reroute + dual mla/lid cache over ALL layers,
            //     no layer filter - the pre-merge behavior our sparse stack expects)
            //   LLAMA_GLM_SPARSE=0 (explicitly) -> pre-merge dense default:
            //     fall through to the standard cache (dense attention escape hatch)
            //   unset -> upstream's native glm-dsa path below (native DSA cache
            //     for the trunk + plain cache for the NextN/MTP context)
            {
                ...
                if (e != nullptr) {
                    // explicitly disabled: pre-merge dense default (standard cache)
                    goto llm_memory_default;
                }
