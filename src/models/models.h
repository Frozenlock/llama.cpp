...
struct llama_model_glm_dsa : public llama_model_base {
    ...
    // native glm-dsa graphs (upstream). This fork's env-gated paths reroute in
    // build_arch_graph: LLAMA_GLM_SPARSE!=0 -> llama_model_deepseek32::graph
    // (trained-sparse/IndexShare stack), LLAMA_GLM_MTP -> llama_model_deepseek2::graph_mtp
    // (the pre-merge draft-head binding the spec stack was validated against).
    struct graph : public llm_graph_context {
        graph(const llama_model & model, const llm_graph_params & params);
    };
