...
static void get_rows_cuda_float(
    ...
    if constexpr (std::is_same<src0_t, dst_t>::value) {
        ...
        // the vectorized fast path has no shard rebase/zero-fill; fall through
        // to the generic kernel when position-sharded gather is active
        const bool can_vec = VEC > 1 && enough_blocks && shard_rows == 0 &&
            (ne00 % VEC == 0) &&
...
static void ggml_cuda_get_rows_switch_src0_type(
        ...
        const size_t nb1, const size_t nb2, const size_t nb3,
        cudaStream_t stream, const int64_t shard_base = 0, const int64_t shard_rows = 0) {
    // position-sharded gather (rebase + zero-fill) is implemented only in the
    // float and legacy-quant kernels; the K-quant/IQ fast path (get_rows_cuda_kq)
    // would silently gather wrong rows, so refuse it. In practice the sharded
    // DSA cache is f16/f32/bf16 or q8_0.
    GGML_ASSERT(shard_rows == 0 ||
        src0_type == GGML_TYPE_F16  || src0_type == GGML_TYPE_F32  ||
        src0_type == GGML_TYPE_BF16 || src0_type == GGML_TYPE_I32  ||
        src0_type == GGML_TYPE_Q1_0 || src0_type == GGML_TYPE_Q2_0 ||
        src0_type == GGML_TYPE_Q4_0 || src0_type == GGML_TYPE_Q4_1 ||
